...
def ramachandran_plot(atomgroup, selection, outputfile1, outputfile2, image_format='png'):
    ...
    data = {'phi_mean':a[:, 0].mean(),'phi_std':a[:, 0].std(), 'psi_mean':a[:, 1].mean(),'psi_std':a[:, 1].std()}
    #print("phi_mean, phi_stddev, psi_mean, psi_stddev")
    print('{d[phi_mean]} {d[phi_std]} {d[psi_mean]} {d[psi_std]}'.format(d=data))
    # open hdf file
    ...
    plt.savefig(outputfile1, format=image_format, dpi=600)
    ...
    with sns.axes_style("white"):
        ...
        plt.savefig(outputfile2, format=image_format, bbox_inches='tight', dpi=600)
...
template = template_environment.get_template('ramachandran_html.j2')
